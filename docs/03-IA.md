...
|---|---|---|---|---|
| 1 | `TAB-WEEK` | 이번 주 / This Week | 최신 주간 기록 상태와 핵심 CTA | `CMP-12 SevenStitchRail` 축소형 |
| 2 | `TAB-ARCHIVE` | 지난 주들 / Weeks | 저장된 기록 탐색 | `rectangle.stack` 후보 |
| 3 | `TAB-SETTINGS` | 설정 / Settings | 권한, 알림, Plus, 개인정보 | `gearshape` |
...
| `SHEET-PAY-01` | Plus Paywall | Full-height sheet | 유료 가치, 구매, 복원 | UC-10, UC-11 |
...
6. sticky primary CTA: `이 n장 남기기`

레이아웃:

- 7장: 큰 hero 1장 + 중간 2장 + 작은 4장 조합이 production 기본입니다.
...
- share, edit, like, filter, comment, metadata overflow는 V1에 없습니다.

### `SHEET-REP-01` Replace Photo

- 제목: `어떤 사진으로 바꿀까요?`
- 후보를 동일 크기 2-column grid로 표시
...
- primary: `기록 보기`
- secondary: `완료`
- 최초 저장이고 알림 미결정이면 다음으로 `SHEET-NOT-01`을 제안합니다.
...
- V1에는 overflow, 편집, 공유, 내보내기, 삭제 menu 없음
- photo tap → `SCR-WK-04`
...
1. `사진`: full/limited/denied 상태, 관리
2. `알림`: on/off/undetermined, 시스템 설정
...
| 알림 primer | Sheet | 완료 흐름을 방해하지 않는 선택 |
| paywall | Full-height sheet | 닫으면 정확한 원래 상태로 복귀 |
...
| Permission | 앱이 데이터에 접근할 수 없음 | 설정 열기/사진 선택 관리 |
| Loading | 작업 진행 중 | 취소 |
