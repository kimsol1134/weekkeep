...
| 2 | `TAB-ARCHIVE` | 지난 주들 / Weeks | 저장된 기록 탐색 | `CMP-13 SemanticTabIcon` stacked album/pages silhouette |
| 3 | `TAB-SETTINGS` | 설정 / Settings | 권한, 알림, Plus, 개인정보 | `CMP-13 SemanticTabIcon` adjustment-sliders silhouette |
...
| `SCR-SET-01` | Settings | Tab root | 권한·알림·Plus·로컬 저장·지원 상태 | UC-03, UC-11, UC-13 |
| `SCR-SET-02` | Privacy | Push | 기기 내 처리·외부 전송·보존 한계 설명 | UC-13 |
| `SCR-SET-03` | About & Support | Push | 지원, 약관, 버전, acknowledgements | UC-13 |
...
3. `지난 7일에서 최대 7장을 골라드려요. 마음에 안 드는 사진만 바꾸고 저장하면 끝이에요.`
4. privacy badge: `사진은 이 iPhone에서만 살펴봐요`
5. primary CTA: `지난 7일 남기기`
...
| 4 | `welcomePending(accessScope)` | Photos 사용 가능 + Welcome 미저장 | 지난 7일을 남겨볼까요? | 만들기 | 없음 |
| 5 | `preRegularWaiting` | Welcome 저장 + 아직 완료된 첫 Regular target 없음 | 다음 기록을 기다리고 있어요 | 없음 | 지난 주들 보기 |
| 6 | `saved(albumID)` | 현재 target 존재 + 같은 `weekKey` 저장 기록 존재 | 지난주를 남겼어요 | 기록 보기 | 지난 주들 보기 |
| 7 | `noEligiblePhotos(accessScope)` | 현재 target 미저장 + 접근 가능한 적격 사진 0장 | 지난주에는 남길 사진이 없어요 | limited면 사진 선택 관리, full이면 다시 확인 | 개인정보 보기 |
| 8 | `entitlementLocked` | 현재 target 미저장 + 적격 사진 1장 이상 + 무료 한도 도달 + Plus 비활성 확인 | 지난 두 주를 잘 남겼어요 | Plus로 계속하기 | 지난 기록 보기 |
| 9 | `ready(accessScope, photoCount)` | 현재 target 미저장 + 적격 사진 1장 이상 + 생성 권한 있음 | 지난주를 남겨볼까요? | 7장 초안 만들기 | 날짜/접근 상태 |

`preRegularWaiting`에서만 아직 첫 Regular target을 기다립니다. 인위적 countdown이나 연속 기록 streak는 보여주지 않습니다. 저장된 현재 target은 완료 창이 끝날 때까지 `saved`를 유지하며 `preRegularWaiting`으로 되돌리지 않습니다.
...
1. Welcome Week를 저장할 때 `regularCycleStartsAt`을 다음 월요일 00:00으로 설정합니다. 이는 첫 Regular Week의 **source week start**입니다.
2. `weekStart >= regularCycleStartsAt`인 첫 전체 캘린더 주가 끝나기 전에는 `preRegularWaiting`입니다. Welcome Week와 겹치는 직전 완료 주를 다시 제안하지 않습니다.
...
2. 현재 단계: `사진을 모으는 중` → `비슷한 사진을 정리하는 중` → `7장 초안을 준비하는 중`
3. determinate가 가능한 경우 `분석한 사진 x/y`
4. 보조 문구: `사진은 이 iPhone을 떠나지 않아요`
...
2. 7장 Regular Week는 `지난주, 이 7장을 남길까요?`, Welcome Week는 `지난 7일, 이 7장을 남길까요?`; 1–6장은 실제 `n장`으로 치환
3. `SevenPhotoGrid` 또는 실제 개수 grid
4. 현재 선택 photo와 `이 사진 바꾸기`; 각 셀의 직접 접근 가능한 viewer·교체 action
5. `n장의 사진` 및 제한/부분 결과 안내
6. grid와 helper/replacement action, quiet privacy note 뒤의 normal-flow primary CTA: `이 n장 남기기`
...
- Regular Week 제목: `지난주가 한 장의 앨범이 됐어요.`
- primary: `이번 주를 공유하기` (local share preparation)
- secondary: `기록 보기`
- tertiary: `완료`
- 최초 저장이고 알림 미결정이면 다음으로 `SHEET-NOT-01`을 제안합니다. 이미 알림 상태가 결정되었거나 primer를 본 경우에는 system prompt를 다시 열지 않고 저장 완료로 진행합니다.
...
- 7장은 canonical wordmark, warm paper, date range, hero+2+4, exact seven muted stitch palette, restrained `Made with Weekkeep` signature를 사용합니다. 1–6장은 실제 사진만 adaptive layout에 배치합니다.
- share button을 명시적으로 누르기 전에는 native share sheet를 열지 않습니다. temporary file은 share activity가 끝나면 정리합니다.
- recipient, destination, filename, location, score, child/family identity, analytics payload를 artifact나 analytics에 넣지 않습니다. V1 이미지에는 public install URL을 그리지 않습니다.

### `SHEET-NOT-01` Notification Primer

- 제목: `매주 월요일 저녁에 알려드릴까요?`
- 설명: `지난주를 남길 때가 되면 가볍게 알려드릴게요.`
- primary: `알림 받기`
- secondary: `지금은 괜찮아요`
- 알림 본문: `지난주를 1분 만에 남겨볼까요?`
...
2. `앞으로의 주들도 계속 남겨요.`
3. 포함 가치: 무제한 주간 기록, 현재 앱에 남아 있는 기존 기록 열람은 이미 무료
...
1. `사진`: full/limited는 접근 관리, denied는 Settings 회복, restricted는 정책 설명만, notDetermined는 Photos 요청
2. `알림`: 저장 기록이 0개이면 `첫 기록 저장 후` 상태와 설명만 표시하고 permission action은 숨김; 1개 이상이면 `notDetermined`만 `알림 켜기`, 그 외 상태는 `알림 설정 열기`
3. `Weekkeep Plus`: active/free, 구매 또는 복원
4. `데이터 저장`: 현재 iPhone 로컬 저장, 별도 백업 없음, 앱 삭제·기기 변경 시 유실 가능성
5. `개인정보`: 기기 내 처리 요약, 상세
6. `도움말`: 문의, 약관
7. footer: app version/build
...
1. **분석 위치:** 사진 분석은 이 iPhone에서 실행됩니다.
2. **외부 전송:** 사진·썸네일·사진 식별자는 분석 SDK로 보내지 않습니다.
3. **저장 위치:** Weekkeep의 주차·선택 순서는 현재 앱 설치에 로컬 저장됩니다.
4. **보존 한계:** `Weekkeep은 별도 백업을 제공하지 않아 앱을 삭제하거나 기기를 바꾸면 기록이 사라질 수 있어요. 원본 사진은 사진 앱에 남아 있어요.`
5. **구매 복원:** Plus 이용 권한만 복원하며 주간 기록은 복원하지 않습니다.
