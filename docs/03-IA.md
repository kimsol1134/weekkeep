...
- recipient, destination, filename, location, score, child/family identity, analytics payload를 artifact나 analytics에 넣지 않습니다. footer prompt는 `How was your family's week?` / `너희 가족의 이번 주는 어땠어?`로 7-stitch 위에만 배치하며, V1 이미지에는 public install URL, QR code, loud ad overlay를 그리지 않습니다. 기존의 restrained `Made with Weekkeep` branding을 유지합니다. 이 next-candidate는 제출된 build 6에 포함되지 않았고 공개 릴리스 전 URL이 live라고 주장하지 않습니다.
