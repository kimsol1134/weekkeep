...
| 기준일 | 2026-08-07 |
| 상태 | Approved |
| 구현 | Core implemented and tested / remote build 3 is uploaded, `VALID`, attached, and in the current Apple review submission; remote build 4 is `VALID` but unattached; local candidate build 6 is configured in `project.yml` and not uploaded; local build 5 remains historical evidence; App Privacy is published for remote build 3 and Apple review is `WAITING_FOR_REVIEW`; public source repository verification is `Validated`; approval, public release, and later external gates remain pending |
...
| Physical-device install/launch | `release/local/target-device-qa/20260806T2102KST-build3/`에서 corrected build `1.0.0 (3)`의 fixture-safe Debug arm64 app이 paired physical `iPhone 16 Pro`에 in-place 설치 성공. build 3의 `-ui-fixtures -ui-fixtures-skip-notification` argument launch command가 unlocked 상태에서 성공 반환했으며, 이후 공식 XCTest 캡처 재실행은 device re-lock/backlight-off preflight에서 중단됨. QuickTime iPhone source는 인식했지만 preview/recording frame은 회색이라 footage로 인정하지 않음 | Partial — build 3 install/launch-command evidence만 검증; physical XCTest UI attachments, target-device footage, 실제 library 성능·icon QA는 pending; Photos/notification 권한과 private pixels는 변경하지 않음 |
...
| Release configuration | `scripts/validate-release.sh --build` and fresh Release archive/IPA on 2026-08-06; build-3 distribution IPA is `1.0.0 (3)` with analytics `NO`, purchases `YES`, empty PostHog token, injected RevenueCat public key, Light UI, `get-task-allow=false`, and no fixture launch markers | Pass — ASC build 3 is `VALID`, attached, and in the current `WAITING_FOR_REVIEW` submission; key value is never recorded |
...
| Public web surfaces | `/`, `/privacy`, `/terms`, `/support`; lint, 7 server-render tests, production build, dependency audit 0 vulnerabilities, mobile/desktop visual check; logged-out HTTP verification | Pass — existing Sites project version 4 is public at `https://weekkeep-app.kimsol1134.chatgpt.site` (deployment `2026-08-06T09:35:40.860943+00:00`); all four routes returned 200, rendered `kimsol1134@gmail.com`, and had zero `kimsol1134@naver.com` matches |
| Public source repository — 2026-08-07 | `https://github.com/kimsol1134/weekkeep` is public and logged-out reachable; repository URL HTTP 200; raw `https://raw.githubusercontent.com/kimsol1134/weekkeep/main/LICENSE` HTTP 200; GitHub recognizes root `LICENSE` as MIT; checked at `2026-08-07T07:03:22+09:00`; default branch `main`; unauthenticated `git ls-remote main` commit `af1faab05739e95c5ffb2645d4e0ad396c8d97b3` | Validated — source availability and logged-out verification are closed in `release/shipaton-manifest.json`; no App Store approval or public app release is implied |
...
| Build 3 review evidence — 2026-08-06 | ASC app `6798449478`, version `1.0.0`, build `3`, ID `81697c0b-a68e-482a-be6f-50806e56fbff`; distribution IPA, archive, screenshot, and polling evidence are in ignored `release/local/asc-release-build3-20260806-onboarding-rerun1/` | Pass — ASC processing `VALID`, build 3 is attached, canonical en-US/ko screenshot sets are remotely `COMPLETE`, and replacement submission `88c157ee-ce87-41c3-8a4a-71e614993a58` is `WAITING_FOR_REVIEW` |
| ASC metadata and availability — 2026-08-06 | Categories Photo & Video/Lifestyle, content rights `DOES_NOT_USE_THIRD_PARTY_CONTENT`, all-none age questionnaire, app free price, USA/KOR app availability, IAP USA/KOR availability, public support/privacy URLs, copyright `© 2026 Sol Kim`, and final review contact | Pass — App Privacy is published; IAP product/version are `WAITING_FOR_REVIEW` with the current review screenshot; app version `1.0.0` build `3` remains manual release |
| App Privacy publication — 2026-08-06 | Published data usage answers are exactly: Customer Support = App Functionality, linked to user, no tracking; User ID = App Functionality, not linked, no tracking; Purchase History = App Functionality, not linked, no tracking | Pass — App Privacy is published with exactly these three entries |
| Review submission boundary — 2026-08-06 | Current IOS review submission `88c157ee-ce87-41c3-8a4a-71e614993a58`; exactly 2 `READY` items: `appStoreVersion` `ac4f183e-1019-4ffc-827f-f5514f0d349b` and IAP version `cedd0fe9-5b2a-478e-a58f-9ae2269ecd7f`; submitted at `2026-08-06T11:47:54.203Z` | `WAITING_FOR_REVIEW` — app version `1.0.0` build `3` remains manual release; prior submission `260abf1a-886b-4014-8262-541d261e60e2` is historical `COMPLETE`; approval and public release are not claimed |
| Remote build 4 state — 2026-08-07 | App Store Connect truth supplied for this pass: version `1.0.0`, build `4`, processing `VALID`, not attached; build identifier is not recorded locally | Known external truth only — no upload or attachment action was performed |
...
| Local build 6 release-candidate evidence — 2026-08-07 | `project.yml` `CURRENT_PROJECT_VERSION: "6"`; new local Settings visual-QA evidence `release/local/visual-qa/20260807-build6-notification-settings-rerun1/final/` | Pass — FR-015 notification-settings evidence is local-only; build 6 is not uploaded, attached, submitted, reviewed, approved, or released; the historical build-5 six-screen App Store set remains separately labeled |
...
Apple Developer bundle ID와 App Store distribution profile은 생성했고, App Store Connect app record와 build 3의 processing/attachment를 확인했다. corrected build-3 en-US/ko screenshot six-set을 ASC에 업로드해 각 locale six assets, `COMPLETE`, `1320×2868` 및 canonical checksum을 재확인했다. IAP `weekkeep_plus_lifetime`는 ASC에 US $19.99로 존재하고 product/version state는 `WAITING_FOR_REVIEW`다. Final App Review contact를 구성했고, App Privacy data-usage 답변은 2026-08-06에 게시되었다. current review submission `88c157ee-ce87-41c3-8a4a-71e614993a58`에는 `appStoreVersion` `ac4f183e-1019-4ffc-827f-f5514f0d349b`와 IAP version `cedd0fe9-5b2a-478e-a58f-9ae2269ecd7f`의 정확히 2개 `READY` item이 포함되었고, 2026-08-06T11:47:54.203Z에 제출되어 `WAITING_FOR_REVIEW` 상태다. App version 1.0.0 build 3은 manual release로 유지한다. Previous build-2 submission and attachment are retained as historical evidence. RevenueCat dashboard mapping은 owner-authenticated configuration으로 기록하고 sandbox purchase/restore는 pending으로 유지한다. Public policy/support site는 기존 Sites project의 version 4로 공개 배포·로그아웃 검증을 완료했다. `Shipaton Judge Access 2026` offer는 app `6798449478` / IAP `6798491084`에 ID `bb4f7fd6-2b08-4aa7-9f55-e140a3e94e28`로 구성되었고, `USA:FREE`·`KOR:FREE` 및 세 customer eligibility가 확인되었다. SANDBOX one-time-code batch ID `7791e39d-f462-4647-be72-e00c75a50eff`는 10개·`2026-10-31` 만료로 생성되었지만 redemption은 테스트하지 않았다. 현재 IAP product/version은 `WAITING_FOR_REVIEW`이므로 production judge code는 app이 `Ready for Distribution`에 도달하고 IAP가 `Approved`가 될 때까지 차단한다. US public App Store release, Apple approval, Shipaton video upload, redemption/production judge code delivery, Devpost submission은 계속 대기 중이다. PostHog는 V1 Release에서 의도적으로 비활성이다.

Build 3 was archived/exported locally, uploaded as ASC build `81697c0b-a68e-482a-be6f-50806e56fbff`, processed `VALID`, attached to version 1.0.0, and submitted in the replacement review submission. Its local evidence is `release/local/asc-release-build3-20260806-onboarding-rerun1/RELEASE-EVIDENCE.md`; remote screenshot replacement evidence is recorded in the after-state snapshot under `release/local/`.
...
| `RB-03` | App Store Connect app record | `6798449478`와 version `1.0.0` record/version ID가 authenticated API에서 확인됨; remote build 3 attachment is current review state, remote build 4 is valid but unattached, local build 6 is not uploaded, local build 5 remains historical, public listing is pending | `Weekkeep`, `com.solkim.weekkeep`, SKU, build 3/4/5/6 state separation, metadata and release evidence |
...
| `RB-05` | 최종 배포 | remote build 3 IPA가 distribution-sign되어 ASC `VALID` 처리·version attachment 완료; remote build 4 is valid but unattached; local build 6 candidate has local screenshots and is not uploaded; local build 5 remains historical; final contact/review detail, corrected build-3 screenshot six-sets, IAP review screenshot 확인; App Privacy published for remote build 3; review `WAITING_FOR_REVIEW`, approval·public release pending | App Review approval → manual release/public URL; local build-6 upload/attachment remains an explicit next gate; no approval or public release is claimed |
| `RB-06` | 대회 자산 | metadata/script와 canonical Remotion composition contract, 로컬 final MP4 render 및 cut/caption QA 완료; 한국어·영어 각 4장의 exact `1179×2556`, no-alpha fixture 로컬 검증 완료; Shipaton offer 구성과 10개 SANDBOX batch 생성은 확인했지만 redemption·production judge code delivery, 공개 upload·로그아웃 재생·target-device footage, App Store marketing artwork·Devpost receipt는 미완료 | [Shipaton checklist](11-SHIPATON-SUBMISSION.md#11-최종-제출-체크리스트) 전체 완료 |
| `RB-08` | 공개 소스 저장소와 라이선스 | root `LICENSE`의 MIT/Sol Kim/2026 local validation과 ignored-source scan, `https://github.com/kimsol1134/weekkeep`의 public visibility/source availability/logged-out verification, raw `LICENSE` HTTP 200, GitHub MIT 인식을 모두 확인 | Validated — checked at `2026-08-07T07:03:22+09:00`; default branch `main`, unauthenticated `git ls-remote main` commit `af1faab05739e95c5ffb2645d4e0ad396c8d97b3`; manifest evidence is redacted and contains no local/private paths |
| `RB-07` | Build 3 review replacement | build 3 archive/export, distribution IPA inspection, local App Store screenshot regeneration, ASC upload/`VALID` processing, version attachment, remote screenshot replacement, and exact-two-item submission are complete; build 2 remains historical, and the approved 72-second video is unchanged | App Review outcome, manual release, and external Shipaton gates remain pending |
