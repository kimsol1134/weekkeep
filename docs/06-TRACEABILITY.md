...
| 기준일 | 2026-08-07 |
| 상태 | Approved |
| 구현 | Core implemented and tested / ASC build 6 is uploaded, `VALID`, attached to version `ac4f183e-1019-4ffc-827f-f5514f0d349b`, and in the current Apple review submission; build 4 is `VALID` but unattached; builds 1–5 remain historical/non-target evidence; App Privacy is published for the current 1.0.0 app version and Apple review is `WAITING_FOR_REVIEW`; public source repository verification and the official YouTube demo with logged-out playback/duration verification are `Validated`; approval, public release, purchase/restore, judge, target-device, Devpost, and later external gates remain pending |
