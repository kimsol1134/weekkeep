...
| 기준일 | 2026-08-07 |
| 상태 | Approved |
| 구현 | Core implemented and tested / remote build 3 is uploaded, `VALID`, attached, and in the current Apple review submission; remote build 4 is `VALID` but unattached; local candidate build 6 is configured in `project.yml` and not uploaded; local build 5 remains historical evidence; App Privacy is published for remote build 3 and Apple review is `WAITING_FOR_REVIEW`; approval, public release, public source repository, and later external gates remain pending |
...
| `RB-08` | 공개 소스 저장소와 라이선스 | root `LICENSE`의 MIT/Sol Kim/2026 local validation과 ignored-source scan은 통과했지만 public repository URL, source availability, visibility, logged-out verification은 실제 공개 전 pending | authorized public GitHub publication 후 logged-out source/license check와 manifest transition |
