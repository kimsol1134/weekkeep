...
| Current share/release verification — 2026-08-07 | Dedicated `Weekkeep Shipaton Build7 iPhone 15 Pro` iOS 26.5 simulator; full unit suite, regular fixture UI suite, share renderer regression, localization, screenshot, core-only and full release validators | Pass — `WeekkeepTests` 143/143; `WeekkeepUITests` 21 executed, 4 skipped by existing opt-in/physical-capture guards, 0 failures; `WeeklyAlbumShareRendererTests` 18/18; localization and selected four-screen screenshot contracts passed; `scripts/validate-release.sh --shipaton-core-only` and full `scripts/validate-release.sh` passed with 3 external-state warnings and no project generation |

Apple Developer bundle ID와 App Store distribution profile은 생성했고, App Store Connect app record와 current build 6의 processing/attachment를 확인했다. TestFlight internal group `Weekkeep Internal QA`는 build 6 `READY_FOR_BETA_TESTING`과 verified account-holder tester의 `INVITED` 상태까지 확인했지만 설치·purchase test는 확인하지 않았다. Historical build-3 en-US/ko screenshot six-set과 build-5 local candidate evidence는 각각 역사적 evidence로 보존한다. IAP `6798491084` / `weekkeep_plus_lifetime`는 ASC에 US $19.99로 존재하고 version `cedd0fe9-5b2a-478e-a58f-9ae2269ecd7f`와 product/version state는 `WAITING_FOR_REVIEW`다. Final App Review contact를 구성했고, App Privacy data-usage 답변은 2026-08-06에 현재 `1.0.0` 앱 버전 범위로 게시되었다. Build 6의 로컬 IPA 검증은 `PrivacyInfo` 포함만 확인하며 별도 build-6 privacy publication event를 뜻하지 않는다. current review submission `a9b0a18f-6cf6-4af4-8e6f-c77009831e00`에는 `appStoreVersion` `ac4f183e-1019-4ffc-827f-f5514f0d349b`와 IAP version `cedd0fe9-5b2a-478e-a58f-9ae2269ecd7f`의 정확히 2개 `READY_FOR_REVIEW` item이 포함되었고, 2026-08-06T22:44:38.573Z에 제출되어 `WAITING_FOR_REVIEW` 상태다. App version 1.0.0 build 6은 manual release로 유지한다. Previous build-2/build-3 submissions and attachments are retained as historical evidence. RevenueCat dashboard mapping은 owner-authenticated configuration으로 기록하고 sandbox purchase/restore는 pending으로 유지한다. Public policy/support site는 기존 Sites project의 version 4로 공개 배포·로그아웃 검증을 완료했다. `Shipaton Judge Access 2026` offer는 app `6798449478` / IAP `6798491084`에 ID `bb4f7fd6-2b08-4aa7-9f55-e140a3e94e28`로 USA/KOR free configuration과 세 customer eligibility로 유지되고 있다. SANDBOX one-time-code batch ID `7791e39d-f462-4647-be72-e00c75a50eff`는 10개·`2026-10-31` 만료로 생성되었지만 redemption은 테스트하지 않았다. Production custom code `WEEKKEEPJUDGES` 시도는 parent IAP 미승인으로 거절되었고 production code는 생성되지 않았다. 현재 IAP product/version이 `WAITING_FOR_REVIEW`이므로 production judge code는 app이 `Ready for Distribution`에 도달하고 IAP가 `Approved`가 될 때까지 차단한다. 공식 public YouTube demo upload와 로그아웃 playback/duration verification은 2026-08-07에 `Validated`되었다. US public App Store release, Apple approval, redemption/production judge code delivery, target-device footage, Devpost submission은 계속 대기 중이다. GitHub Release backup URL은 공개 다운로드 보조 증거로 보존하며, 공식 영상 gate는 [Shipaton Submission SSOT](11-SHIPATON-SUBMISSION.md#7-72-second-demo-master)와 manifest가 소유한다. PostHog는 V1 Release에서 의도적으로 비활성이다.
...
This is not remote ASC build 6, local build 7, production PhotoKit evidence, or Shipaton submission screenshot evidence. The ready capture visibly places the primary start CTA below the initial viewport, contradicting the current no-scroll-first-CTA requirement in this build-6 evidence. The separate build-7 screenshot record below resolves that finding for the local DEBUG fixture only; native share/share delivery, purchase/restore, actual PhotoKit performance, physical video, public release, and Devpost submission remain unclaimed; replacing remote build 6 requires explicit user authorization.
