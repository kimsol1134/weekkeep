...
description: >
  ...
  are identical and sacred — the green/pink/cream editorial triad, LINE Seed Sans KR at weight 500
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "LINE Seed Sans KR", cqw: 1.56, weight: 400, lineHeight: 1.38 }
  body-card:{ fontFamily: "LINE Seed Sans KR", cqw: 1.35, weight: 400, lineHeight: 1.34 }
  ...
  # — display ramp (LINE Seed Sans KR weight 500, opsz, negative tracking) —
  title-card-sm:{ fontFamily: "LINE Seed Sans KR", cqw: 2.9, weight: 400, lineHeight: 0.98, tracking: "-0.01em" }
  title-card:{ fontFamily: "LINE Seed Sans KR", cqw: 3.5, weight: 400, lineHeight: 0.96, tracking: "-0.01em" }
  headline:{ fontFamily: "LINE Seed Sans KR", cqw: 4.4, weight: 400, lineHeight: 1.0, tracking: "-0.02em" }
  headline-xl:{ fontFamily: "LINE Seed Sans KR", cqw: 5.0, weight: 400, lineHeight: 0.96, tracking: "-0.02em" }
  display:{ fontFamily: "LINE Seed Sans KR", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.02em" }
  display-hero:{ fontFamily: "LINE Seed Sans KR", cqw: 11.5, weight: 400, lineHeight: 0.92, tracking: "-0.02em" }
  stat-figure:{ fontFamily: "LINE Seed Sans KR", cqw: 11.5, weight: 400, lineHeight: 0.92, tracking: "-0.03em" }
  stat-figure-unit:{ fontFamily: "LINE Seed Sans KR", cqw: 5.7, weight: 400, lineHeight: 0.92 }
  name:    { fontFamily: "LINE Seed Sans KR", cqw: 2.3, weight: 700, lineHeight: 1.0 }
...
- **Fonts** — already set to **LINE Seed Sans KR** (display) / **LINE Seed Sans KR** (body); ignore any preset font name lingering in prose.
...
Penguin classic or a quiet annual report. One confident voice, **LINE Seed Sans KR at weight 500**
...
- **LINE Seed Sans KR weight 500** (opsz, negative tracking) for all display; **serif 400** body; **JetBrains Mono 500** uppercase chrome.
...
- **Squint** — one LINE Seed Sans KR moment dominates at 3–6× its neighbor; the surface block centers the eye.
...
Two ramps. The **reading/chrome ramp** (LINE Seed Sans KR **400** body 1.56cqw; JetBrains Mono labels
in px) carries copy + chrome; the **display ramp** (LINE Seed Sans KR **500** `title-card-sm` 2.9cqw →
...
**Ground** `{colors.green}`, `slide-pad`. **Composes** topbar (mono label + monogram-circle), display-hero, body lede, footline. **Focal** a 2-line LINE Seed Sans KR `display-hero` in pink (one word in cream), left. **Chrome** mono topbar; mono footline. **Accent** pink display + the monogram circle. **Silence** ~50%. **Fixed** serif 500, flat, monogram 2px pink. **Free** title, which word is cream. **Density** low.
...
**Ground** `{colors.green}`, `slide-pad`. **Composes** topbar, kpi-block (mono tag + 220px stat-figure + serif description), footline. **Focal** a LINE Seed Sans KR `stat-figure` (~11.5cqw, +unit) in pink over a 2px pink rule. **Chrome** mono topbar + footline. **Accent** the pink figure. **Silence** ~55%. **Fixed** serif 500 figure, 2px accent rule, flat. **Free** the figure (from script), tag, description. **Density** low.

### 4 · Statement (quote · move: display serif · cream · left)

**Ground** `{colors.cream}`, generous pad. **Composes** topbar, display, name (600), caption-mono role. **Focal** a 2-line LINE Seed Sans KR `display` (~7.3cqw) in green; a 600-weight attribution name + mono role beneath. **Chrome** mono topbar. **Accent** none — the serif carries it. **Silence** ~55%. **Fixed** serif 500 display + 600 name only, no italic. **Free** quote, name, role. **Density** low.
...
- Run every display in **LINE Seed Sans KR weight 500**, opsz, negative-tracked, tight line-height; **body at 400**.
...
- **Type** — LINE Seed Sans KR **500** display (opsz, negative-tracked) / **400** body; mono uppercase 0.08–0.18em; ≥1.4cqw floor.
...
- **LINE Seed Sans KR (opsz 8..60) + JetBrains Mono via Google Fonts** — the optical-size axis is critical; a non-opsz fallback flattens the size-aware letterforms. CJK: LXGW WenKai (display) / Noto Serif SC (body) / Noto Sans Mono CJK (chrome).
